...
async def _grade_answer(
    ...
    # For fill_blank and short_answer, use LLM to judge — single call with structured output
    chunk_section = ("【课程内容】\n" + chunk_text + "\n\n") if chunk_text else ""
    system_msg = (
        "你是一位耐心的编程助教。请判断学生回答并给出点评。\n"
        "填空题：学生填的内容是否与标准答案语义一致或等效 → 正确\n"
        "简答题：学生理解核心概念，解释基本正确（允许合理误差） → 正确\n"
        "回答为空/敷衍/无关 → 错误\n"
        "【输出格式 - 严格按以下JSON输出，不要其他内容】\n"
        '{"correct":true或false,"feedback":"点评内容（正确时2-3句，错误时4-6句含标准答案）"}'
    )
    user_msg = (
        chunk_section +
        "【题目】" + question + "\n"
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            if is_correct:
                feedback = "回答正确！" + feedback if feedback else "回答正确！扎实地掌握了本题的核心知识点，继续加油！"
            return {"correct": is_correct, "feedback": feedback[:800]}
    except Exception:
        pass
    # Fallback
